...
from multilevel_pg.multilevelpg.logger import logger, tabular
import tensorflow as tf
from multilevel_pg.multilevelpg.samplers.sampler import Sampler
...
class BiSampler(Sampler):
    ...
    def sample(self, explore=False):
        ...
        mix_observe_2 = np.hstack((tf.one_hot(self._current_observation_n[2], self.env.num_state),
                                   tf.one_hot(action_1, self.env.action_num)))
        if explore_this_step:
            self._epsilon = self._epsilon * self._epsilon_decay
            action_2 = np.array([np.random.randint(self.env.action_num)])
        else:
            action_2 = self.agents[2].act(mix_observe_2)
        supplied_observation.append(mix_observe_2)
        action_n.append(action_2)

        
        mix_observe_3 = np.hstack((tf.one_hot(self._current_observation_n[3], self.env.num_state),
                                   tf.one_hot(action_2, self.env.action_num)))
        if explore_this_step:
            self._epsilon = self._epsilon * self._epsilon_decay
            action_3 = np.array([np.random.randint(self.env.action_num)])
        else:
            action_3 = self.agents[3].act(mix_observe_3)
        supplied_observation.append(mix_observe_3)
        action_n.append(action_3)

        mix_observe_4 = np.hstack((tf.one_hot(self._current_observation_n[4], self.env.num_state),
                                   tf.one_hot(action_3, self.env.action_num)))
        if explore_this_step:
            self._epsilon = self._epsilon * self._epsilon_decay
            action_4 = np.array([np.random.randint(self.env.action_num)])
        else:
            action_4 = self.agents[4].act(mix_observe_4)
        supplied_observation.append(mix_observe_4)
        action_n.append(action_4)
